arr512 = [

]


arr32 = [
0xf26cb481,0x16a5dc92,0x3c5ba924,0x79b65248,0x2fc64b18,0x615acd29,0xc3b59a42,0x976b2584,
0x6cf281b4,0xa51692dc,0x5b3c24a9,0xb6794852,0xc62f184b,0x5a6129cd,0xb5c3429a,0x6b978425,
0xb481f26c,0xdc9216a5,0xa9243c5b,0x524879b6,0x4b182fc6,0xcd29615a,0x9a42c3b5,0x2584976b,
0x81b46cf2,0x92dca516,0x24a95b3c,0x4852b679,0x184bc62f,0x29cd5a61,0x429ab5c3,0x84256b97
]


targetstr = "Hire me!!!!!!!!"
target16 = [0]*16
for i in range(15):
    char = targetstr[i:i+1]
    target16[i] = ord(char)


def ch(val):
    v = hex(val)
    v = v.replace("0x", "")
    v = v.zfill(2)
    v = "0x" + v +","
    return v


def th(val):
    v = hex(val)
    v = v.replace("0x", "")
    v = v.zfill(2)
    return v


goodpairs = {}
for n in range(16):
    print("target16[", n, "]=", hex(target16[n]), chr(target16[n]))
    pairs = []
    notfound_byte1_cnt = 0
    all_found_pairs = 0
    for byte1 in range(256):
        found_cnt = 0
        for byte2 in range(256):
            if arr512[byte1] ^ arr512[256+byte2] == target16[n]:
                found_cnt += 1
                all_found_pairs += 1
                pairs.append([byte1, byte2])
        if not found_cnt:
            notfound_byte1_cnt += 1
    goodpairs[n] = pairs

of = open("n_data.inc", "w")

of.write("unsigned char arr512[512]={\n")
rcnt = 0
rstr = ""
for i in range(512):
    rstr += ch(arr512[i])
    rcnt += 1
    if rcnt == 32:
        if i > 500:
            rstr = rstr.rstrip(",")
        of.write(rstr+"\n")
        rcnt = 0
        rstr = ""
of.write("};\n\n")


rev1 = [-1]*256
rev2 = [-1]*256
for i in range(256):
    n = arr512[i]
    if not rev1[n] == -1:
        rev2[n] = i
    else:
        rev1[n] = i
banned1 = [0]*256
for i in range(256):
    if rev1[i] == -1:
        banned1[i] = 1


of.write("unsigned char banned[256]={\n")
rcnt = 0
rstr = ""
for i in range(256):
    rstr += str(banned1[i])+","
    rcnt += 1
    if rcnt == 64:
        if i > 250:
            rstr = rstr.rstrip(",")
        of.write(rstr+"\n")
        rcnt = 0
        rstr = ""
of.write("};\n\n")


of.write("unsigned char rev1[256]={\n")
rcnt = 0
rstr = ""
for i in range(256):
    v = rev1[i]
    if v < 0:
        v = 0
    rstr += ch(v)
    rcnt += 1
    if rcnt == 32:
        if i > 250:
            rstr = rstr.rstrip(",")
        of.write(rstr+"\n")
        rcnt = 0
        rstr = ""
of.write("};\n\n")


of.write("unsigned char rev2[256]={\n")
rcnt = 0
rstr = ""
for i in range(256):
    v = rev2[i]
    if v < 0:
        v = 0
    rstr += ch(v)
    rcnt += 1
    if rcnt == 32:
        if i > 250:
            rstr = rstr.rstrip(",")
        of.write(rstr+"\n")
        rcnt = 0
        rstr = ""
of.write("};\n\n")


of.write("unsigned char goodpairs[8192]={\n") # 16*16*16*2 = 8192
for i in range(16):

    for r in range(16):
        rstr = ""
        for c in range(16):
            gp = goodpairs[i][r*16+c]
            rstr += ch(gp[0])
            rstr += ch(gp[1])
        if i == 15 and r == 15:
            rstr = rstr.rstrip(",")
        of.write(rstr+"\n")
of.write("};\n\n")


of.close()


xorlist = {}
for i in range(32):
    nlist = []
    for n in range(32):
        if i == n:
            continue
        if arr32[i] & (2**n) > 0:
            nlist.append(n)
    xorlist[i] = nlist


of = open("n_xor.inc", "w")
for i in range(32):
    of.write("\ta = checklist[checklistpointer].check32[" + str(i) + "];\n")
    for k in range(14):
        of.write("\ta ^= temp32[" + str(xorlist[i][k]) + "];\n")
    of.write("\tif (banned[a] == 1) {\n")
    of.write("\t\tchecklistpointer--;\n")
    of.write("\t\tcontinue;\n")
    of.write("\t}\n")
    of.write("\tchecklist[checklistpointer].check32[" + str(i) + "] = a;\n")
of.close()


of = open("n_xor_test.inc", "w")
for i in range(32):
    of.write("        i = input32[" + str(i) + "];\n")
    for k in range(14):
        of.write("        i ^= temp32[" + str(xorlist[i][k]) + "];\n")
    of.write("        input32[" + str(i) + "] = i;\n")
of.close()

